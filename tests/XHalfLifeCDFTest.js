...
contract("XHalfLifeCDF", ([alice, bob, minter]) => {
  beforeEach(async () => {
    ...
    this.halflifecdf = await XHalfLifeCDF.new({ from: alice });
  ...
  context("should create and return balance successfully", async () => {
    ...
    it("should return balance of the stream", async () => {
      await this.xdex.approve(this.halflifecdf.address, "1000", {
        from: alice,
      });
      await this.halflifecdf.createStream(
        this.xdex.address,
        bob,
        "60",
        "140",
      ...
      console.log(
        "give bob a stream: startBlock #140, stopBlock #160, depositAmount 60"
      );

      await time.advanceBlockTo("140");
      //return sender balance of the stream
      assert.equal(
        (await this.halflifecdf.balanceOf("1", alice)).toString(),
        "60"
      );
      //return recipient balance of the stream
      assert.equal(
        (await this.halflifecdf.balanceOf("1", bob)).toString(),
        "0"
      );
